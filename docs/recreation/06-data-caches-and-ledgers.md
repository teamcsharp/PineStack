...
| `script_ledger.jsonl`, `script_ledger_seq.json` | **the pillar**: the running order as WRITTEN, keyed `(block, ord)` — assigned once and never rewritten. A round is committed before it is audible; **every other heard line** — gold bars, the SFX guy's quips, adverts, rescue fills, which were 49% of one measured hour — is committed the moment it has been HEARD (#1339), one block each, marked `scripted: false`. So it covers every line that reached the air, not only round material, and the flag says which were planned. The SFX guy inside a round sits at the position he was rolled into. Everything else in this table records what happened; this records what was going to | `script_ledger_commit` (from `_speak_turns_floorless`, before the weld goes to air) and `script_ledger_catch_up` (from `airlog_keeper`, after the fact) → `screenplay_compose`, the SCRIPT view | two days (`SCRIPT_LEDGER_KEEP_S`, 8 MB cap) — regenerable-but-lossy: wipe it and the hour reverts to being ordered by `air_at`, which is the thing it exists to replace |
