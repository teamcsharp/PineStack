...
function decide(settings, listeners, options) {
  ...
  /* ONE ROOM BY DEFAULT, SEVERAL ON PURPOSE.
   *
   * An earlier cut of this resolved two switched-on devices down to one, on
   * the rule "individually but never at the same time". That was aimed at
   * the wrong thing, and it is worth writing down why.
   *
   * What produced two rooms ACCIDENTALLY was never this table: it was the
   * station routing to `here` with no solo owner, so every page that
   * happened to be open sounded at once, a few hundred milliseconds apart.
   * That is fixed where it lives - in the destination picker, which claims
   * the air for exactly one page (#1008 /api/radio/solo).
   *
   * This table is the operator's EXPLICIT statement. Plain click sets one
   * device; shift-click adds another, deliberately: "make sure i can shift
   * + click to map audio to multiple devices." Two rows saying play means
   * he said so, with a modifier key, on purpose - and silently overriding
   * that is worse than either outcome.
   *
   * So the table is obeyed as written, and `singular` REPORTS rather than
   * enforces. */
  ...
  for (const row of live) {
    ...
    row.why = wanting.length > 1
      ? 'switched on and present - one of ' + wanting.length
        + ' rooms you chose'
