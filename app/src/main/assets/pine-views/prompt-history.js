(function (root) {
  ...
  function mount(host) {
    ...
    function requestRow(row) {
      var d = make('details', 'ph-call'); d.dataset.id = row.id; var summary = make('summary'); summary.appendChild(make('b', '', row.model + ' / ' + row.purpose)); summary.appendChild(make('time', '', new Date(row.at * 1000).toLocaleString() + ' / ' + row.state)); summary.appendChild(make('span', '', row.preview)); var actions = make('span', 'ph-call-actions'); actions.appendChild(command('Open prompt execution in node view', 'c:chart--network', function (event) { event.preventDefault(); event.stopPropagation(); fetchRecord(row, nodeView); })); actions.appendChild(command('Open prompt conversation', 'c:chat', function (event) { event.preventDefault(); event.stopPropagation(); fetchRecord(row, conversation); })); summary.appendChild(actions); d.appendChild(summary); d.addEventListener('toggle', function () { if (d.open) fetchRecord(row, function (got) { renderRequest(d, got); }); }); return d;
    }
    function load() {
