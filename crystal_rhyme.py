...
# Voicing and manner fold; L and R do NOT fold into each other - calm (K AA L M)
# against harm (H AA R M) is the pinned refusal of #1076, and folding the
# liquids would accept it.
_NEAR_CLASS = {}
for _grp in (("F", "V", "TH", "DH"), ("S", "Z", "SH", "ZH"), ("T", "D"), ("P", "B"), ("K", "G"),
             ("M", "N", "NG"), ("CH", "JH")):
    for _p in _grp:
        _NEAR_CLASS[_p] = _grp[0]


def near_tails(ta, tb) -> bool:
    """2026-09-08: the slant landing the ear accepts and neither reading
    proved - the same final stressed vowel, and codas equal once voicing and
    manner are folded (proof/move, breathe/perceive) or differing by one
    inserted consonant (down/found, lit/shift, stay/ways). A liquid (L/R) is
    not an insertion the ear forgives, so calm/harm and being/feeling stay
    refused, as tests/test_rejections_learning_2026_09_08 pins."""
    ta, tb = tuple(ta), tuple(tb)
    if not ta or not tb or ta[0] != tb[0] or ta == tb:
        return False
    ca = [_NEAR_CLASS.get(p, p) for p in ta[1:]]
    cb = [_NEAR_CLASS.get(p, p) for p in tb[1:]]
    if ca == cb:
        return True
    if abs(len(ca) - len(cb)) == 1:
        lo, hi = (ca, cb) if len(ca) < len(cb) else (cb, ca)
        return any(hi[:i] + hi[i + 1:] == lo and hi[i] not in ("L", "R")
                   for i in range(len(hi)))
    return False


def terminal_near_rhymes(bars, *, normalize=None, excluded=()) -> dict:
    """Near pairs between bar landings (see near_tails), reported kind="near"."""
    normalize = normalize or (lambda word: re.sub(r"[^a-z]", "", word.lower()))
    excluded = set(excluded)
    ends = []
    for position, bar in enumerate(bars):
        ...
        if word in excluded or len(normal) < 2:
            continue
        prons = [(tail, phones) for phones, tail, _n in _pronunciations(word)]
        if prons:
            ends.append((position, word, normal, prons))
    matches, seen = [], set()
    for i in range(len(ends)):
        for j in range(i + 1, len(ends)):
            ...
            if ni == nj or pair in seen:
                continue
            hit = next(((ta, tb) for ta, _a in pri for tb, _b in prj if near_tails(ta, tb)), None)
            if hit:
                seen.add(pair)
                matches.append({"words": [wi, wj], "bars": [pi, pj], "kind": "near",
                                "rhyme_phones": [list(hit[0]), list(hit[1])]})
                if len(matches) == 4:
                    return _result(matches)
    return _result(matches)


def rhymes_with(word: str, other: str) -> str:
    """2026-09-08: "" when the two words do not rhyme, else "perfect" or
    "near" - the reading the repair hint must use before it suggests a word.
    Measured on the evening's stored hints: 64% of the suggestions were not
    rhymes at all, because they were picked by the spelling reader alone."""
    a, b = str(word or "").lower().strip("'"), str(other or "").lower().strip("'")
    if not a or not b or a == b:
        return ""
    pa, pb = _pronunciations(a), _pronunciations(b)
    if not pa or not pb:
        return ""
    for _phones, ta, _n in pa:
        for _p2, tb, _n2 in pb:
            if ta == tb:
                return "perfect"
    for _phones, ta, _n in pa:
        for _p2, tb, _n2 in pb:
